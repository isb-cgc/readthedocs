.. ISB-CGC documentation master file, created by
   sphinx-quickstart on Sun Dec 20 11:20:02 2015.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

The ISB Cancer Genomics Cloud (CGC) 
===================================

Welcome to the ISB-CGC Cookbook on Read the Docs.

Here you will find documentation describing the features of the ISB-CGC platform,
and details about the data that we are hosting on the Google Cloud Platform.

This documentation is a work-in-progress, please let us know how we can improve it!

Contents:

.. toctree::
   :maxdepth: 2

   sections/About-ISB-CGC
   sections/Web-UI
   sections/Programmatic-API
   sections/TCGA-Data
   sections/Reference-Data
   sections/FAQ
   sections/Support
   sections/Other-Useful-Links

Indices and tables
==================

* :ref:`search`
* :ref:`genindex`


.. * :ref:`modindex`


