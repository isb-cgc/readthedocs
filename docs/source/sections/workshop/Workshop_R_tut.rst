...
.. code-block:: r
	...
	project_short_name=c('TCGA-CESC','TCGA-HNSC')
...
.. code-block:: r
	...
	sqlQuery = paste("SELECT case_barcode, project_short_name, hpv_calls, hpv_status ",
	                 "FROM ", clinical_table,
	                 " WHERE project_short_name in (",paste(shQuote(project_short_name),collapse = ','),")",sep="")
	...
	ggplot(data=hpv_table, aes(x=hpv_status, fill=project_short_name)) + geom_bar(stat="count", position=position_dodge())
...
.. code-block:: r
	...
	mean_affected_genes$xlabel <- paste0(mean_affected_genes$project_short_name, "_", mean_affected_genes$HGNC_gene_symbol)
...
.. code-block:: r
	...
	#qplot(data=gexp_affected_genes, x=project_short_name, y=normalized_count, col=HGNC_gene_symbol, geom="boxplot")
	#qplot(data=gexp_affected_genes, x=project_short_name, y=log2(normalized_count), col=HGNC_gene_symbol, geom="boxplot")
	qplot(data=gexp_affected_genes, x=log2(normalized_count+1), col=HGNC_gene_symbol, geom="density") + facet_wrap(~ project_short_name)
...
.. code-block:: r
	...
	res0 <- gxps %>%
	group_by(project_short_name, HGNC_gene_symbol) %>%
	...
	res1 <- merge(x=top5, y=gxps) %>% mutate( project_short_name_Gene = paste0(project_short_name, "_", HGNC_gene_symbol))

	# now we can plot the results...
	ggplot(res1, aes(x=project_short_name_Gene, y=log2(normalized_count+1), fill=hpv_status)) + geom_boxplot()
...
.. code-block:: r
	...
	SELECT
	  ...
	  p.project_short_name AS disease,
	  p.x AS x,
	  p.sx2 AS sx2,
	  p.nx AS nx,
	  o.y AS y,
	...
	FROM (
	  ...
	  WHERE
		...
		# selecting the cases... could also previously put this in a table
	    SELECT
	      case_barcode
	    FROM
	      [isb-cgc:TCGA_bioclin_v0.Clinical]
