...
Once a user has `uploaded their own data`_ to the Webapp, that data can be visualized using the same graphing tools that are available for graphing TCGA and/or TARGET data.  However, the process for graphing user data is slightly different from how it is done with TCGA data.
