...
You can find the maf(s) file of interest using the GDCquery_Maf function within TCGAbiolinks. In the GDC, MAFs
generated by different variant callers are saved in different files.  In the example below, we are querying for the LAML MAF file generated using the muse variant caller. This will search the latest release of the GDC database for MAF file generated using the muse variant caller from the TCGA-LAML project. When it locates the file, it will download it to your R workspace as a dataframe allowing you to visualize the contents of the maf file as a tab-delimited table.
...
Now let's query directly the ISB-CGC BigQuery somatic maf table
###############  
