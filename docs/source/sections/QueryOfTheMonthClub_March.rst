...
.. code-block:: r
  ...
  #generated by different variant callers are saved in different files. To get all MAfs associated with a particular cancer 
  #type, you will have to specificy the caller name. In the example below, we are querying for the LAML MAF file generated
  #using the muse variant caller. This will search the latest release of the GDC database for MAF file generated using the   
  #muse variant caller from the TCGA-LAML project. When it locates the file, it will download it to your R workspace as      
  #dataframe allowing you to visualize the contents of the maf file as a tab-delimited table.
