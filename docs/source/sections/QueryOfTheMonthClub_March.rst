...
First, let's use TCGAbiolinks to retrieve maf files of interest from the GDC 
###############

You can find the maf(s) file of interest using the GDCquery_Maf function within TCGAbiolinks package. In the GDC, MAFs
generated by different variant callers are saved as different files.  In the example below, we are querying the latest releae of the GDC database for a MAF file generated using the muse variant caller from the TCGA-LAML project. When it locates the file, it will download it to your R workspace as a dataframe allowing you to visualize the contents of the maf file as a tab-delimited table.

.. code-block:: r

  
  laml.muse.maf <- GDCquery_Maf("LAML", pipelines = "muse")
  #view the contents of this maf file
  head(laml.muse.maf)

Now let's query the ISB-CGC BigQuery somatic maf table
###############  
There are a number of ISB-CGC open-access table, including for somatic maf files. In ISB-CGC, we have concatenated all of the open-accesss MAF files for all projects have been concatenated into a large, query-able BigQuery table. Instead of downloading single files at a time, you can query this entire datatable. You will need your Google Cloud Project ID to query the BigQuery table. 

.. code-block:: r
  
  myprojectID = "your_project_id"
  sql1<-"SELECT Hugo_Symbol, Chromosome, Start_Position, End_Position, Reference_Allele,
  Tumor_Seq_Allele2,Variant_Classification, Variant_Type, Tumor_Sample_Barcode FROM 
 `isb-cgc.TCGA_hg38_data_v0.Somatic_Mutation` WHERE project_short_name = 'TCGA-LAML'"
